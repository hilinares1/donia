...
class ProductTemplate(models.Model):
    ...
    def _search_dr_has_discount(self, operator, value):
        ...
        if pricelist_id:
            discounted_products, catch_date = self._get_discounted_products(pricelist_id)
    ...
    def _get_discounted_products(self, pricelist_id):
        products = self.search([('sale_ok', '=', True)])
        product_ids = []
        for product in products:
            if product._get_combination_info(only_template=True).get('has_discounted_price'):
                product_ids.append(product.id)
        return product_ids, fields.Datetime.to_string(fields.Datetime.now())
